...
| 2026-07-07 | Added full `README.md` (overview, features, tech stack, setup, scripts, API reference, security, deployment) and `docs/ARCHITECTURE.md` (layered design, data model, auth, transaction lifecycle, admin console, integrations, scaling notes). |
| 2026-07-08 | Added investment plans as a core feature: `src/lib/plans.ts` (Startup/Professional/Premium/Unlimited with min/max/ROI + assets), `PlansSection` component rendered on the homepage (`#plans`). Also produced a self-contained Replit MVP at `replit/index.html` (plans, invest modal with range validation + projected returns, localStorage portfolio, mock wallet connect). |
...
| 2026-07-08 | Migrated the store to Convex (real-time backend, no Supabase): `convex/schema.ts` + `users/sessions/transactions/plans` functions, `src/lib/convexServer.ts` (ConvexHttpClient), `src/lib/store.ts` rewritten as a Convex-backed adapter, `ConvexClientProvider` in root layout, dashboard overview converted to a real-time client component via `useQuery`, and wallet-connect persisted to profiles via `useMutation`. `_generated` is stubbed until `npx convex dev`. Seed script + `.env.example` updated for Convex. |
| 2026-07-08 | Upgraded investment plans content (Solana crypto / Agriculture / Real Estate / Car stock+Altcoin+BONK+XRP+Meme) with Tabler icons on the homepage `PlansSection`; `convex/plans.ts` seed synced. Made `/admin` server-access-controlled (redirect 307 without admin cookie; `/api/admin/*` already returns 403 for non-admins via `assertAdmin`). Added Tawk.to (`src/components/tawk-to.tsx`, gated by `NEXT_PUBLIC_TAWKTO_ID`) to layout and Replit MVP. Replit `index.html` updated with new plans, Tabler icons, and Tawk.to — designated as the dev/hosting artifact. |
